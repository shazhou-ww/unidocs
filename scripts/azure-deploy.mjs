 ...
 *
 * `--cas-access-key` 只在 Key Vault 里还没有该 secret 时需要(首次部署)。
 */
...
/** 三个服务镜像 + Gateway migration + 通用 Doc migration。 */
export const IMAGES = [
...
export function parseArgs(argv) {
  const args = { ...DEFAULTS, casBaseUrl: "", casAccessKey: "", skipBuild: false, requireCas: false };
...
/**
 * `CAS_ACCESS_KEY` 与 Postgres 密码性质**不同**,不能共用 `seedSecret()`:
 * 它不是本轮新生成的密钥,而是**已经存在于 Cloudflare 侧、本轮必须对齐**的
 * 既有密钥。`packages/cloudflare-cas/src/worker.ts` 对每个请求校验
 * CAS access key 不匹配就 401,而 docx 的图片路径经
 * `packages/doctype-server-common/src/cas-client.ts` 发出去的正是 Azure 侧的这个值。
 * 现场随机生成一个只会让所有跨云 CAS 请求 401。
 *
 * 本地栈由 `scripts/doc-types.mjs` 显式给两边注入同一 CAS key。
 */
function resolveCasAccessKey(keyVaultName, provided) {
  const existing = tryCapture("az", [
    "keyvault", "secret", "show",
    "--vault-name", keyVaultName,
    "-n", CAS_ACCESS_KEY_SECRET,
    "--query", "value",
    "-o", "tsv",
  ]);
  if (existing) {
  ...
  if (!provided) {
    throw new Error(
      ...
      "This value is NOT generated by this deployment: it must equal the CAS_ACCESS_KEY of the " +
  ...
  run(
    ...
    {},
    `az keyvault secret set --vault-name ${keyVaultName} -n ${CAS_ACCESS_KEY_SECRET}`,
  );
  return provided;
}
...
function seedSecrets(keyVaultName, args) {
  ...
  const casAccessKey = resolveCasAccessKey(keyVaultName, args.casAccessKey);
  const markdownAccessKey = seedSecret(keyVaultName, MARKDOWN_ACCESS_KEY_SECRET, 48);
  const docxAccessKey = seedSecret(keyVaultName, DOCX_ACCESS_KEY_SECRET, 48);
  return { pgAdminPassword, casAccessKey, markdownAccessKey, docxAccessKey };
}

/**
 * Step 4:在 ACR 里构建五个镜像;`--skip-build` 时跳过,只复用已有 tag。
 *
...
function buildAndPushImages(args, bootstrap, tag) {
  ...
  console.log("[4/7] building 5 linux/amd64 images in ACR for tag", tag);
...
function deployMain(args, secrets, tag) {
  ...
  //
  // 两条命令的 args 里都直接带着 secure 参数的明文
  ...
  return {
    ...
    migrateJobNames: outputs.migrateJobNames.value,
  };
}
 ...
 *
 * `jobNames` 来自 Step 5 的 Bicep output，不在脚本里复制资源名。
 */
...
async function runMigrations(args, jobNames) {
  for (const jobName of jobNames) await runMigration(args, jobName);
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  await runMigrations(args, mainOutputs.migrateJobNames);
