...
import { TenantApiOperationOrder, TenantApiTagGroups } from "./openapi.js";
...
export function renderTenantApiReferenceHtml(document: OpenAPI.Document): string {
  const content = serializeForInlineScript(document);
  const tagOrder = serializeForInlineScript(TenantApiTagGroups.flatMap((group) => group.tags));
  const operationOrder = serializeForInlineScript(TenantApiOperationOrder);
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <title>UniCAS Tenant API</title>
  <style>html, body, #app { height: 100%; margin: 0; } body { background: #fff; }</style>
</head>
<body>
  <div id="app"></div>
  <noscript>JavaScript is required to render the UniCAS Tenant API reference.</noscript>
