/**
 * @unicas/cli — Unicas control-plane management CLI.
 *
 * Programmatic surface: interactive OAuth login, token persistence, remote MCP
 * client, the tool catalog, and the stdio MCP server. The `unicas` binary in
 * `src/cli.ts` wires these together.
 */

export { loadConfig, parseScopes, CONTROL_PLANE_SCOPES } from "./config.js";
export type { CliConfig, ControlPlaneScope } from "./config.js";
export { TokenStore } from "./store.js";
export type { PersistedSession, TokenStoreOptions } from "./store.js";
export { CliError } from "./errors.js";
export { PersistentOAuthClientProvider, NeedsLoginError } from "./oauth/provider.js";
export type { PersistentOAuthClientProviderOptions } from "./oauth/provider.js";
export { runLoginFlow, openBrowser } from "./oauth/login.js";
export type { LoginFlowOptions, LoginFlowResult } from "./oauth/login.js";
export { revokeAndClearSession } from "./oauth/logout.js";
export type { LogoutOptions, LogoutResult } from "./oauth/logout.js";
export { UnicasRemoteClient, UNICAS_CLI_VERSION } from "./remote/client.js";
export type { RemoteClientOptions, ToolCallResult } from "./remote/client.js";
export { TOOL_CATALOG, getToolDefinition, generateIdempotencyKey } from "./mcp/catalog.js";
export type { ToolDefinition, ControlPlaneToolScope } from "./mcp/catalog.js";
export { runMcpStdioServer } from "./mcp/stdio-server.js";
export type { McpStdioServerOptions } from "./mcp/stdio-server.js";
export { main } from "./cli.js";
