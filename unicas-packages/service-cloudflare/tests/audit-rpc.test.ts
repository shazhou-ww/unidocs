...
vi.mock("@unicas/control-plane-mcp", () => ({
  ...
  createControlPlaneMcpWorker: () => ({ fetch: () => new Response("mcp") }),
}));
...
function rpcRequest(path: string, key = READER_KEY): Request {
  const headers: Record<string, string> = {
    "X-Test-Audit-Target": `https://cas.example${path}`,
  };
  if (key !== null) headers["X-Test-Audit-Reader-Key"] = key;
  return new Request("https://cas.example/admin/_audit-test", { headers });
}
...
describe("audit-reader RPC", () => {
  ...
  test("the RPC only accepts GET", async () => {
    const env = await createEnv();
    const response = await worker.fetch(new Request("https://cas.example/admin/_audit-test", {
      method: "POST",
      headers: {
        "X-Test-Audit-Target": `https://cas.example/_internal/audit/refs?stackId=${STACK}&refDomain=${DOMAIN}`,
        "X-Test-Audit-Reader-Key": READER_KEY,
      },
    }), env);
