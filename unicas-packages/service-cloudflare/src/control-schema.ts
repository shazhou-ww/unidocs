...
const CONTROL_TABLE_MIGRATIONS = [
  ...
  "CREATE TABLE IF NOT EXISTS cas_playground_file_roots (stack_id TEXT NOT NULL, owner_key TEXT NOT NULL, root_id TEXT NOT NULL, name TEXT NOT NULL, manifest_hash TEXT NOT NULL, revision INTEGER NOT NULL DEFAULT 1, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, PRIMARY KEY (stack_id, owner_key, root_id))",
...
const CONTROL_INDEX_MIGRATIONS = [
  ...
  "CREATE INDEX IF NOT EXISTS cas_playground_file_roots_by_owner ON cas_playground_file_roots(stack_id, owner_key, name, root_id)",
  ...
  "CREATE INDEX IF NOT EXISTS cas_managed_issuers_by_status ON cas_stack_managed_issuers(status, stack_id)",
...
export const CONTROL_SCHEMA_MIGRATIONS = [
  ...
  "DROP TABLE IF EXISTS cas_stack_oauth_issuer_keys",
...
export async function migrateControlSchema(db: D1Database): Promise<void> {
  ...
  await ensureColumns(db, "cas_stack_oauth_issuers", [["mode", "TEXT NOT NULL DEFAULT 'external'"]]);
  await db.exec("INSERT OR IGNORE INTO cas_stack_managed_issuers (stack_id, issuer, audience, metadata_url, authorization_endpoint, token_endpoint, jwks_uri, scopes_supported, code_challenge_methods_supported, status, verified_at, jwks_digest, capability_max_lifetime_seconds, revision) SELECT oauth.stack_id, oauth.issuer, oauth.audience, oauth.metadata_url, oauth.authorization_endpoint, oauth.token_endpoint, oauth.jwks_uri, oauth.scopes_supported, oauth.code_challenge_methods_supported, CASE WHEN oauth.status = 'active' THEN 'active' ELSE 'disabled' END, COALESCE(oauth.verified_at, stacks.created_at, 0), oauth.jwks_digest, oauth.capability_max_lifetime_seconds, oauth.revision FROM cas_stack_oauth_issuers AS oauth LEFT JOIN cas_stacks AS stacks ON stacks.stack_id = oauth.stack_id WHERE oauth.mode = 'managed'");
  await db.exec("DELETE FROM cas_stack_oauth_issuers WHERE mode = 'managed'");
  await db.exec("UPDATE cas_stack_managed_issuers SET capability_max_lifetime_seconds = 3600, revision = revision + 1 WHERE capability_max_lifetime_seconds = 120");
