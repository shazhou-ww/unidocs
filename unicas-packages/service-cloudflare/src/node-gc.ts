...
import type { NodeGcCandidate, NodeGcDeletion, NodeGcRepository, NodeGcScope } from "@unicas/service";
...
export class CloudflareNodeGcRepository implements NodeGcRepository {
  ...
  async readStillExpiredUnreferenced(scope: NodeGcScope, hash: string, expiresAtOrBefore: number): Promise<NodeGcDeletion | null> {
    ...
    return {
      ...
      childReferences: edges.results.map((edge) => ({ hash: edge.child_hash, count: edge.cnt })),
    };
  }
  ...
  async commitDeletion(scope: NodeGcScope, deletion: NodeGcDeletion): Promise<void> {
    const batch: D1PreparedStatement[] = [
      this.db.prepare("DELETE FROM cas_edges WHERE stack_id = ? AND tenant_id = ? AND parent_hash = ?")
        .bind(scope.stackId, scope.tenantId, deletion.hash),
      this.db.prepare("DELETE FROM cas_nodes WHERE stack_id = ? AND tenant_id = ? AND hash = ?")
        .bind(scope.stackId, scope.tenantId, deletion.hash),
    ];
