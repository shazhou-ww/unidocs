...
export function createViewBundlesHttp(repository: ViewBundleRepository, objectStore: BundleObjectStore, bundleOrigin: string) {
  const contract = adminApiContract.viewBundles;
  const implementation = implement(contract).$context<{ admin: AdminContext; requestId: string }>();
  const service = createViewBundleService(repository, objectStore, { bundleOrigin });
  const router = {
    ...
    get: implementation.get.handler(({ input, context }) => service.get(context.admin, input.params.viewBundleId)),
    updateMetadata: implementation.updateMetadata.handler(({ input, context }) => service.updateMetadata(context.admin, input.params.viewBundleId, input.body,
  ...
  function operationError(error: unknown, requestId: string): Response | null {
    ...
    if (error instanceof ViewBundleOperationError) return Response.json({
  ...
  return async (request: Request, admin: AdminContext, requestId: string): Promise<Response> => {
    ...
    const collection = url.pathname === "/admin/api/v1/view-bundles";
    ...
    if (request.method === "POST" && collection) {
      try {
        if (!request.body) throw new ViewBundleOperationError("invalid_request");
    ...
    const handler = new OpenAPIHandler(router, {
      ...
      interceptors: [async ({ next }) => {
        try {
          ...
          if (error instanceof ViewBundleOperationError) throw new ORPCError(error.code.toUpperCase(), {
            status: { invalid_request: 400, not_found: 404, idempotency_conflict: 409, precondition_failed: 412, forbidden: 403, bundle_already_exists: 409 }[error.code],
