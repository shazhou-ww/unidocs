 ...
 * same way `stacks/unidocs-azure/local/runtime.mjs` spawns the azure-gateway/azure-markdown
 ...
 * process `stacks/unidocs-azure/local/runtime.mjs` runs in — has to stay responsive to
 ...
 * with `node` directly). Mirrors `stacks/unidocs-azure/local/runtime.mjs`'s
...
export async function teardown(): Promise<void> {
  await stopAzurite();
  // `-v` matches `stacks/unidocs-azure/local/runtime.mjs`'s teardown: without it, every run
