 ...
 * separate from tenant node data (tenant tables live in `cloudflare-cas`).
 * Migrations are idempotent (CREATE TABLE/INDEX IF NOT EXISTS) and are run by
 * the deployable `cas-admin-webui` at startup, mirroring the tenant schema
 * pattern.
 */
