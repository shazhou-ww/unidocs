 ...
 * JWK by signing a server-issued one-time challenge (see service
 * `createPossessionChallenge`). `possessionProof` is a compact JWS whose
 * payload is the UTF-8 challenge string; the service verifies it with the
 * submitted public JWK using the declared algorithm.
 *
...
import { isSupportedKeyAlgorithm } from "./validation.js";
