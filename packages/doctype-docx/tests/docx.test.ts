...
describe("createDocxDocumentType", () => {
  it("creates, edits, queries, and reloads a DOCX document", async () => {
    ...
    expect(docx.contentType).toBe(DOCX_CONTENT_TYPE);
    await expect(docx.query({ kind: "getText", payload: undefined }, initial)).resolves.toBe("");
    ...
    await expect(docx.query({ kind: "getText", payload: undefined }, initial)).resolves.toBe("");
    await expect(docx.query({ kind: "getParagraphs", payload: undefined }, updated)).resolves.toEqual([
      {
        index: 0,
        text: "Final",
        styleId: "Heading1",
    ...
    const bytes = await docx.save(updated);
    expect(Array.from(bytes.slice(0, 2))).toEqual([0x50, 0x4b]);

    const reloaded = await docx.load(bytes);
    await expect(docx.query({ kind: "getText", payload: undefined }, reloaded))
      .resolves.toBe("Final\nBody");
  });
  ...
  it("does not mutate the input when an operation batch fails", async () => {
    ...
    const before = await docx.save(baseline);
    ...
    await expect(docx.query({ kind: "getText", payload: undefined }, baseline))
      .resolves.toBe("Existing");
    await expect(docx.save(baseline)).resolves.toEqual(before);
  });
});
