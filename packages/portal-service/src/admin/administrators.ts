...
export class AdministratorOperationError extends Error {
  constructor(readonly code: "invalid_request" | "not_found" | "idempotency_conflict" | "administrator_exists" | "precondition_failed" | "cannot_remove_self" | "last_administrator" | "forbidden") {
    super({
      ...
      cannot_remove_self: "An administrator cannot remove their own membership",
      last_administrator: "The final bound administrator cannot be removed",
...
export interface AdministratorRemoveCommand {
  readonly context: AdminContext;
  ...
  return {
    ...
    async remove(context: AdminContext, adminId: string, key: string, expectedEtag: string, requestId: string) {
      ...
      return repository.remove({
        ...
        key,
        expectedEtag,
        fingerprint: await schemaHash({ operation: "removeAdministratorMember", adminId, expectedEtag }),
        audit: {
          auditEventId: id(), actorId: context.memberId, action: "administrator.removed", resourceType: "administrator",
          resourceId: adminId, documentType: null, occurredAt: timestamp, requestId, reason: null,
        },
      });
    },
