...
describe("black & white adjustment", () => {
  it("maps a pure primary to its Photoshop-default gray weight", () => {
    // Default red weight 40% → pure red becomes 0.4 gray ≈ 102, and R=G=B.
    const out = render(doc([solid("bg", [255, 0, 0, 255]), adj("blwh", {})]));
    const [r, g, b] = px0(out);
    expect(r).toBe(g); expect(g).toBe(b);
    expect(r).toBeGreaterThan(95); expect(r).toBeLessThan(110);
  });

  it("respects custom channel weights", () => {
    // Green weight 100% → pure green becomes ~1.0 (white).
    const out = render(doc([solid("bg", [0, 255, 0, 255]), adj("blwh", { greens: 100 })]));
    expect(px0(out)[0]).toBeGreaterThan(250);
  });
});

describe("hue/saturation adjustment", () => {
  it("desaturates via master saturation -100", () => {
    // Pure red → grayscale at its lightness (0.5) ≈ 128, equal channels.
    const out = render(doc([solid("bg", [255, 0, 0, 255]), adj("hue2", { master: { saturation: -100 } })]));
    const [r, g, b] = px0(out);
    expect(r).toBe(g); expect(g).toBe(b);
    expect(r).toBeGreaterThan(120); expect(r).toBeLessThan(136);
  });

  it("colorize turns a gray into a hue", () => {
    // Colorize hue 0 (red), saturation 100 on mid-gray → red.
    const out = render(doc([solid("bg", [128, 128, 128, 255]), adj("hue2", { colorize: true, master: { hue: 0, saturation: 100, lightness: 0 } })]));
    const [r, g, b] = px0(out);
