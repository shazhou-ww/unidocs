...
describe("clipping mask", () => {
  it("confines a clipped layer to the base layer's alpha", async () => {
    ...
    const out = await render(doc(3, 1, [base, clipped]));
...
describe("color overlay effect", () => {
  it("replaces the layer colour within its alpha", async () => {
    ...
    const out = await render(doc(1, 1, [shape]));
