...
describe("load", () => {
  it("reads sample.psd into PsdDoc", async () => {
    ...
    expect(doc.layers.map(l => l.name)).toEqual(["background", "red-box", "ghost-fill"]);
    ...
    // ghost-fill exercises fillOpacity:0 (invisible fill, see generate.mjs).
    const ghost = doc.layers.find(l => l.name === "ghost-fill")!;
    expect(ghost.fillOpacity).toBe(0);
  });
});
