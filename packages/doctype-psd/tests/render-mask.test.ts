...
describe("layer mask", () => {
  it("hides pixels where the mask is black, shows where white", async () => {
...
describe("blend modes (previously fell back to normal)", () => {
  it("difference inverts the backdrop", async () => {
    ...
    const out = await render(doc(1, 1, [back, top]));
