import { Download, Eye, FileText, Layers, LogOut, Monitor, Plus, RefreshCw, Search, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { ApiError, createDocument, downloadDocument, listDocuments, type GatewayDocumentRecord } from "../api.js";
...
export function DocumentsView({ session, onSignedOut }: DocumentsViewProps) {
  ...
  async function create() {
    ...
    busyCreate.current = true; setCreating(true); setError(""); setNotice("");
    try {
      const result = await createDocument(session.tenantId, createType);
      if (!active.current) return;
      if (!result.docId) throw new Error("创建响应缺少作品标识");
      setCreateOpen(false);
      setNotice(result.state === "creating" ? `正在创建 ${typeName(createType)} · ${result.docId}，请稍后刷新列表。`
  ...
  return <div className="workspace-ui">
    ...
    <div className="workspace-desktop">
      ...
      <dialog ref={dialog} aria-labelledby="workspace-create-heading" className="workspace-create" onCancel={event => { event.preventDefault(); closeCreate(); }} onClose={() => { if (!busyCreate.current) setCreateOpen(false); }}>
        <form onSubmit={event => { event.preventDefault(); void create(); }}><div className="workspace-modal-heading"><h2 id="workspace-create-heading">新建作品</h2><button type="button" aria-label="关闭创建窗口" disabled={creating} onClick={closeCreate}><X size={16} /></button></div><div className="workspace-modal-body"><label htmlFor="workspace-create-type">内容类型</label><select id="workspace-create-type" value={createType} disabled={creating} onChange={event => setCreateType(event.target.value)}>{types.map(type => <option value={type} key={type}>{typeName(type)}</option>)}</select></div><div className="workspace-modal-footer"><button type="button" disabled={creating} onClick={closeCreate}>取消</button><button className="workspace-primary" type="submit" disabled={creating || !createType}><Plus size={15} />{creating ? "创建中…" : "创建"}</button></div></form>
      </dialog>
    </div>
  </div>;
}