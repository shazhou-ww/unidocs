...
export interface DiscussionSummary {
  ...
  readonly latestReply: { readonly threadId: string; readonly text: string | null; readonly isPlain: boolean } | null;
}
...
export async function loadDiscussionSummary(
  ...
  let latestReply: DiscussionSummary["latestReply"] = null;
  let latestAt = "";
  for (const { detail, state } of threads) {
    if (state.latestReply === null) continue;
    if (state.latestReply.createdAt <= latestAt) continue;
    latestAt = state.latestReply.createdAt;
    latestReply = { threadId: detail.threadId, text: state.latestReply.content.text, isPlain: state.latestReplyIsPlain };
  }
