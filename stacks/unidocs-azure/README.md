...
`stacks/unidocs-azure/local/`,"删掉 `azure/` 之后 `pnpm test` 仍然全绿"这条不变式就不成立
...
node stacks/unidocs-azure/deploy/deploy.mjs --cas-base-url ... --internal-auth-mode dual --capability-key-id ...
```
...
```bash
node stacks/unidocs-azure/deploy/deploy.mjs                          # 冷启动全量
node stacks/unidocs-azure/deploy/deploy.mjs --bootstrap               # 只 bootstrap
node stacks/unidocs-azure/deploy/deploy.mjs --platform                # 只 Postgres / ACA 环境 / 迁移
node stacks/unidocs-azure/deploy/deploy.mjs --service docx             # 只构建 docx 镜像 + 只部它
node stacks/unidocs-azure/deploy/deploy.mjs --service docx,markdown    # 多选,逗号分隔,镜像并发构建
node stacks/unidocs-azure/deploy/deploy.mjs --gateway                  # 只网关
node stacks/unidocs-azure/deploy/deploy.mjs --service docx --build-concurrency 1   # 覆盖镜像构建并发(默认 2)
```
...
node stacks/unidocs-azure/deploy/deploy.mjs --service markdown &
wait
```
