...
日期：2026-09-07。状态：已部署；真人登录后的生产目录已验证，创建和导出仍待验收。
...
后续真人登录补验（2026-09-07）：用户自行登录后，通过已共享页面读取真实目录，共显示 23 件作品（Markdown 12、DOCX 11），无目录加载错误；当前无 PSD。打开两篇已有 Markdown，均正常显示 v1 与空内容状态，返回列表及手动刷新成功。未创建、编辑、删除或导出作品，未读取或输出凭据。原先的生产目录待验收项已完成，创建／导出和非空内容预览仍待验证。
