...
内置那两套是 Noto Sans 全量（607 KiB）+ Noto Sans SC 按《通用规范汉字表》8105 字
子集化（1.91 MiB）。**单位一律按二进制读**（R19 那道 3 MiB 闸在
`fonts-builtin/tests/generated-index.test.ts` 里写的就是 `3 * 1024 * 1024`）。它存在的理由是**即装即用**：在它之前，`setText` 能不能工作
...
警告。注意它读的是**整张租户登记表**（`readFontIndex` 是一次全表回读，含往次运行、
含别的工具登记的条目），而且只看这一档、看不见内置那一档：内置的中文子集仍然兜着，
所以它现在的含义是"这个租户的登记表里没有中文"，不再是"这个环境排不出中文"。
...
3. **预置脚本的 `--repin`**（§5.4 第 4 条）—— 脚本修不回"索引有条目、根引用是 0"
   这个状态，而那个状态没有任何可观测信号。
4. **按文档子集化** —— 指的是针对某一篇文档实际用到的那些字符现裁一份更小的
   字体。这件事**仍然没做**：租户从 CAS 装的全量 CJK 字体每次仍要拉 5–20 MB，
   靠既有的 `ByteLru` 缓存吃掉。
   注意别和**静态子集化**混起来——那件事已于 2026-09-08 落地（按《通用规范汉字表》
   8105 字把随包发行的中文那套裁到 1.91 MiB，产物提交进仓库，见 §3.4 与
   `packages/fonts-builtin`）。静态那份是一份固定字表、所有文档共用；这一条要的是
   逐文档现裁，两者不互相替代。
