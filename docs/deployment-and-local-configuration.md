...
arguments or printed. `cas-access-key` and `--cas-access-key` are retired in
stack mode; Azure uses its registered stack capability path.
...
under the same stack ID, issuer, and key ID. The deploy defaults match the
current `unidocs-azure` registration except that the active key ID should still
be supplied explicitly during rotation.
