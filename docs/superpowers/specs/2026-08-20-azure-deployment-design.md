...
- **apply 的完整写入顺序**(原设计漏了 CAS 两步,这是重构最易踩坏处):
  ...
  **不变量:409 响应体里的 `version` 必须是服务端当前的版本号,不能是调用方传来的陈旧 `baseVersion`。** 阶段 1 换成主键冲突检测后,撞主键的那个请求不知道自己的 `baseVersion` 已经过期到什么程度,必须重新读一次当前版本才能填对这个字段——现有的并发 apply 测试断言了 `version: 2`,照抄 `baseVersion` 会让这条测试变红。
