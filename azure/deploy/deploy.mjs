 ...
 *
 * `--internal-token` 只在 Key Vault 里还没有该 secret 时才可能需要:
...
/**
 * `INTERNAL_TOKEN` 与 Postgres 密码性质**不同**,不能一概共用 `seedSecret()`:
 * 只要配了 `--cas-base-url`,它就不是本轮新生成的密钥,而是**已经存在于
 * Cloudflare 侧、本轮必须对齐**的既有密钥。`packages/cloudflare-cas/src/worker.ts`
 * 对每个请求校验 `token !== env.INTERNAL_TOKEN` 就 401,而 docx 的图片路径经
 * `packages/doctype-server-common/src/cas-client.ts` 发出去的正是 Azure 侧的这个值。
 * 现场随机生成一个只会让所有跨云 CAS 请求 401。
 *
 * 但如果**没配** `--cas-base-url`(Cloudflare CAS worker 尚未部署的当前形态,
 * 见文件头注释),就没有 Cloudflare 侧需要对齐 —— `INTERNAL_TOKEN` 此时只用
 * 于 Azure 内部 gateway -> doc-type-worker 鉴权,可以放心生成。
 *
 * (本地栈之所以看不出跨云不对齐的问题:`scripts/doc-types.mjs` 硬编码的
 * `INTERNAL_TOKEN = "unidocs-dev-token"` 被 Miniflare 与本地 Azure 栈共用。)
 */
export function decideInternalTokenAction({ existing, provided, casBaseUrl }) {
  if (existing) return "read";
  if (provided) return "write";
  if (!casBaseUrl) return "generate";
  return "error";
}
...
async function resolveInternalToken(keyVaultName, provided, casBaseUrl) {
  ...
  if (action === "read") {
    // 已有则读用 —— 幂等,且第二次部署不需要再传 --internal-token,与是否
    // 配了 --cas-base-url 无关。
    return existing;
  }
  ...
  if (action === "error") {
    throw new Error(
      ...
      "This value is NOT generated by this deployment when --cas-base-url is set: it must equal the " +
      "INTERNAL_TOKEN of the already-deployed Cloudflare CAS worker (packages/cloudflare-cas). That " +
      "worker rejects every request whose X-Internal-Token differs, so a mismatch makes every " +
      "cross-cloud CAS request from azure-docx fail with 401 — the docx image path would break in " +
      ...
      "then rerun with --internal-token <that value>. Or drop --cas-base-url if you don't need the " +
      "Cloudflare CAS worker yet — this script will then generate an Azure-internal-only token.",
    );
  }

  // action is "write" (--internal-token given) or "generate" (no CAS configured,
  // safe to mint a value nothing else needs to match). Either way the value only
  // ever lives in memory / Key Vault, never in a log line.
  const value = action === "write" ? provided : generateSecret(32);

  if (action === "generate") {
    // 提示必须醒目,但绝不能把 token 本体打进日志。
    console.log(
      "[3/7] no --cas-base-url and no existing Key Vault secret: generated a NEW internal-token.\n" +
      "  This token is valid for THIS Azure deployment only (gateway -> doc-type-worker auth).\n" +
      "  If you later wire up the Cloudflare CAS worker, the two sides MUST agree: delete the " +
