...
6. Azure side — no Durable Objects, so no DO bindings to wire up. Instead:
   ...
   - `resolveWorkerUrl()` in `packages/azure-gateway/src/main.ts` and the replica/env wiring in `azure/local/runtime.mjs` already generalise over `docTypes`/`{TYPE}_WORKER_URL` — nothing to change there.
